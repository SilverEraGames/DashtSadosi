...
{
	//We definitely need a valid version
	if (!InFileVersion.IsValid())
	{
		UE_LOG(LogEasyMultiSave, Warning, TEXT("ApplyFileVersionInfo :: File Version is invalid!"));
	}
...
uint32 FSaveFileIntegrity::GetDataHash(const void* Data, const uint64 Length) 
{
	if (!Data)
	{
		UE_LOG(LogEasyMultiSave, Error, TEXT("Data Hash Error"));
		return 0;
	}
...
bool FSaveFileIntegrity::CheckAndSaveFile(const FFileContext& Context, const TArray<uint8>& InData)
{
	...
	const uint32 Hash = GetDataHash(InData.GetData(), InData.Num());
...
bool FSaveFileIntegrity::IsFileHashValid(const TArray<uint8>& RawData, FSaveHeader& OutHeader)
{
	...
	const uint32 CalculatedHash = GetDataHash(PayloadPtr, PayloadSize);
...
EFileValidity FSaveFileIntegrity::LoadRawSaveData(const FFileContext& Context, TArray<uint8>& OutRawData)
{
	...
	//Check minimum size (minimum 4 bytes for a hash)
	if (OutRawData.Num() < sizeof(uint32))
	{
		return EFileValidity::FILE_INVALID;
	}
