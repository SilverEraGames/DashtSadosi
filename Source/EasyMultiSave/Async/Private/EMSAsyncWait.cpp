...
void UEMSAsyncWait::StartWaitTask()
{
	...
	{
		bWorldPartitionInitalizing = EMS->AutoSaveLoadWorldPartition() && !EMS->InitWorldPartitionLoadComplete();
	}
	...
	else
	{
		...
		const bool bWorldPartitionAlreadyComplete = EMS->InitWorldPartitionLoadComplete();
