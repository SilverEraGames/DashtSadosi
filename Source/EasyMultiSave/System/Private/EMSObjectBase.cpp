...
bool UEMSObjectBase::SaveObject(const FString& FullSavePath, UObject* SaveGameObject) const
{
	...
	{
		bSuccess = SaveBinaryArchive(*FullSavePath, Archive);
	}
...
bool UEMSObjectBase::LoadObject(FMemoryReader& FromBinary, UObject* SaveGameObject) const
{
	...
	//Use last file version for objects(they dont need a custom own version)
	ApplyLastFileVersion(MemoryReader);
...
bool UEMSObjectBase::ParseArchiveData(const FLoadArchiveContext& Context, const TArray<uint8>& InData)
{
	...
	//No apply version to package initially
	ApplyLastFileVersion(MemoryReader);
	...
	{
		bWasLoaded = UnpackActorArchive(Context.LoadType, MemoryReader);
	}
	...
	if (!bWasLoaded)
	{
		...
		UE_LOG(LogEasyMultiSave, Log,
			TEXT("Data is valid, but was not parsed for logical reasons like being in a different Level without Multi-Level Saving. File: %s"),
			*FullSavePath);
	}
...
bool UEMSObjectBase::LoadBinaryArchive(const FLoadArchiveContext& Context)
{
	...
	{	
		...
		//Attempt Backup Fallback. Temp will only exist if Move() fails and that never happens
		if (!LoadBinaryData(BackupPath, BinaryData))
		{
			UE_LOG(LogEasyMultiSave, Error, TEXT("Load failed: No valid data in Primary or Backup for %s"), *BackupPath);
			return false;
		}
	...
	//Parse the successfully retrieved data
	const FString DebugPath = bIsRestoredFromBackup ? BackupPath : FullSavePath;
...
bool UEMSObjectBase::LoadBinaryData(const FString& FullSavePath, TArray<uint8>& OutBinaryData) const
{
	...
	if (CheckAndLoadFile(FullSavePath, RawFileData) != EFileValidity::FILE_VALID)
	{
		return false;
	}
...
void UEMSObjectBase::ApplyLastFileVersion(FMemoryReader& MemoryReader) const
{
...
void UEMSObjectBase::SerializeToBinary(UObject* Object, TArray<uint8>& OutData) const
{
	...
	//Write new version info per object
	uint32 ObjectTag = EMS::OBJECT_DATA_TAG;
	uint32 ObjectVersion = EMS::OBJECT_DATA_VERSION;
	MemoryWriter << ObjectTag;
	MemoryWriter << ObjectVersion;
...
void UEMSObjectBase::SerializeFromBinary(UObject* Object, const TArray<uint8>& InData) const
{
	...
	uint32 ObjectTag = 0;
	MemoryReader << ObjectTag;

	//See if versioned
	if (ObjectTag == EMS::OBJECT_DATA_TAG)
	{
		//Can be checked later for additional info
		uint32 ObjectVersion = 0;
		MemoryReader << ObjectVersion;
	...
	else
	{
		...
		/*
		We need to account for old Multi-Level data that was not yet parsed
		The legacy version is read on initial load and then stored again on save
		So we overwrite the actual version here with the legacy one
		*/

		ApplyLastFileVersion(MemoryReader);
		MemoryReader.SetUEVer(LastFileVersion.GetLegacyVer());
	}
...
This is mostly relevant for C++ projects.
**/
 
bool UEMSObjectBase::SaveObjectCollection(const TArray<FRawObjectSaveData>& Objects, const bool bUseSlot) const
{
	if (Objects.IsEmpty())
	{
		return false;
...
bool UEMSObjectBase::ProcessObjectCollection(FMemoryReader& FromBinary, const FSaveObjects& Objects) const
{
